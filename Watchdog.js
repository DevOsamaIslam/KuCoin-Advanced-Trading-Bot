import settings, { timeframes } from "./config/settings.js"
...
import log, { err, verbose } from "./log.js"
...
export default class Watchdog {
  ...
  async collectHistory(watchlist) {
    while (this.history.length < watchlist.length) {
      ...
      if (!history || history.length < 200) {
        ...
        if (history.length < 1441)
          err(`Data not enough for ${pair.symbol}: ${history.length}`)
        continue
      }
      this.history.push({
        ...pair,
  ...
  async enter(options) {
    let { pair, tickerInfo, strategy, order } = options
    ...
    log(
      `${strategy} strategy gives the green light to buy ${pair.symbol} at market value on ${this.tf.text} timeframe`
    )
  ...
  async volSpike(pairs) {
    ...
    for (let pair of pairs) {
      ...
      if (signal) {
        log(
          `RIDE THE WAVE strategy gives the green light to buy ${pair.symbol} at market value on ${this.tf.text} timeframe`
        )
        // buy it
        let order = defineOrder(
          ...
          pair,
          history,
          settings.strategies.RTW.params.risk
        )
        if (!order) continue
