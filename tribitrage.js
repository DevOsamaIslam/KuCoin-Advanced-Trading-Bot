...
const arbitrage = async (options) => {
  ...
  datafeed.subscribe(topic, async (data) => {
    ...
    if (ownInitial > risked * settings.strategies.TRIBITRAGE.diff) {
      ...
      log(
        `Arbitrage opportunity: ${initial}--${getBase(
          symbols.CD
        )}--${median}--${initial}`
      )
      ...
      BC = floor(
        BC * (2 - settings.strategies.TRIBITRAGE.offset),
        getDecimalPlaces(BC)
      )
      CD = floor(
        CD * settings.strategies.TRIBITRAGE.offset,
        getDecimalPlaces(CD)
      )
...
const start = async (options) => {
  let { AB, BC, CD, symbols, ownMedian, risked, target } = options
  let steps = []
  ...
  // step 1 ---------------------------------------------------
  new Trader(steps[0]).tribitrage().then((order) => {
    if (!order) {
...
io.on("order-filled", (order) => {
  ...
  for (const op of opportunities) {
    ...
    if (
      steps[0].pair.symbol == order.symbol &&
      order.clientOid.includes(op.id)
    ) {
      steps[0].order = order
      new Trader(steps[1]).tribitrage().then((order) => {
        if (!order) {
          includeIt(target)
          opportunities.splice(opportunities.indexOf(op), 1)
        }
      })
      break
    }
    // check if the filled order is step 2, then start step 3 and re-enable looking for new arbitrage opportunities
      ...
      steps[1].pair.symbol == order.symbol &&
      order.clientOid.includes(op.id)
    ) {
      steps[1].order = order
      setTimeout(() => {
    ...
    // check if the filled order is step 3, then remove the coin from open opportunities
      ...
      steps[2].pair.symbol == order.symbol &&
      order.clientOid.includes(op.id)
    ) {
      steps[2].order = order
      ...
      revenue += diff
      log(
        `Arbitrage done: ${initial}--${target}--${getBase(
          median
        )}--${initial}: ${floor(diff, 2)} (${floor(revenue, 2)}) ${initial}`
      )
      includeIt(target)
      opportunities.splice(opportunities.indexOf(op), 1)
    }
...
io.on("order-canceled", async (order) => {
  ...
  let oppo = opportunities.find((oppo) => order.clientOid.includes(oppo.id))
  if (oppo) {
    opportunities.splice(opportunities.indexOf(oppo, 1))
  }
})
